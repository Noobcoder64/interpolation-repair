...
def normalizeFormulaSyntax( formula ):
    ...
    formula = re.sub(r"G\(F\(([^\)]*)\)", r"alwEv (\1", formula)
...
for req in e.findall("requirement"):
    ...
    if req.find("kind").text.lstrip().rstrip() == "A":
        lines.append("asm " + normalizeFormulaSyntax(req.find("property").text) + ";\n")
    elif req.find("kind").text.lstrip().rstrip() == "G":
        lines.append("gar " + normalizeFormulaSyntax(req.find("property").text) + ";\n")
file.close()

write_file(lines, directory + specification + ".spectra")
