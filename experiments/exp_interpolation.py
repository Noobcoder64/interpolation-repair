...
experiment_name = f"Interpolation - RANDOM - -1 - NEW PATH EXTRACTION"
...
for spectra_file in spectra_files:
    ...
    with mlflow.start_run(run_name=spectra_file.stem):
        ...
        mlflow.set_tag("benchmark_group", specifications_dir.name)
        ...
        try:
            ...
            mlflow.log_artifact(output_dir / f"{output_file_prefix}_interpolation_nodes.csv")
            mlflow.log_artifact(output_dir / f"{output_file_prefix}_interpolation_params_metrics.csv")

            metrics_file = output_dir / f"{output_file_prefix}_interpolation_params_metrics.csv"
            ...
            if metrics_file.exists():
                try:
                    ...
                    print(f"Error reading or logging metrics from {metrics_file}: {e}")
                    mlflow.set_tag("metric_logging_status", "FAILED")
            else:
                print(f"Metrics file not found: {metrics_file}")
                mlflow.set_tag("metric_logging_status", "MISSING")

            stats_file = output_dir / f"{output_file_prefix}_interpolation_stats.csv"
