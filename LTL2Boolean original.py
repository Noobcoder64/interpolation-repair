import settings
import TextFilesParsing as t
from Counterstrategy import path as p
import re
import pyparsing as pp
import ref_utils.syntax_utils as su
...
def run_settings_example():
    exampledir = settings.basedir+settings.examplename+"/"

    ass_init_file = open(exampledir+settings.examplename+"_Assumptions_Init")
    ass_inv_file = open(exampledir+settings.examplename+"_Assumptions_Inv")
    ass_fair_file = open(exampledir+settings.examplename+"_Assumptions_Fair")

    guar_init_file = open(exampledir+settings.examplename+"_Guarantees_Init")
    guar_inv_file = open(exampledir+settings.examplename+"_Guarantees_Inv")
    guar_fair_file = open(exampledir+settings.examplename+"_Guarantees_Fair")


    path = p.Path(exampledir+"graph_with_signals.dot")

    for i in range(0,settings.unrolling_degree):
        path.unroll()
        print path.unrolling_degree
        print path.unrolled_states[0].id_state
    ...
    pathfile = open(exampledir+"path","w")
    pathfile.write(str(path))
    pathfile.close()

def run_settings_alternative_path_example():
    altpathdir = settings.getAlternativePathDirectory()
    exampledir = settings.getCounterstrategyDirectory()
    ...
    path = p.Path(exampledir+"graph_with_signals.dot",altpathdir+"path")
    ...
    writeMathsatFormulaToFile(altpathdir+"counterstrategy_auto",counterstrategyTranslation)
    writeMathsatFormulaToFile(altpathdir+"guarantees_auto",guaranteesTranslation)
